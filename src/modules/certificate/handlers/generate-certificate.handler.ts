...
export class GenerateCertificateHandler {
  ...
  async handle(query: TelegramBot.CallbackQuery, bot: TelegramBot) {
    ...
    try {
      const certificate = await this.certificateService.generateCertificate(user.telegramId, courseId, language);
      await bot.sendDocument(chatId, certificate.pdfUrl, {
      ...
      await bot.sendMessage(chatId, this.i18nService.getTranslation('errors.certificate_generation_failed', language, { reason: error.message }));
    }
